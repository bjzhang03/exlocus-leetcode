...
// https://blog.csdn.net/xudli/article/details/47015825
	...
	if rowe >= rows && cole >= cols {
		...
		if matrix[rowmid][colmid] == target {
			...
			if binarySearchCols(matrix, target, colmid, rows, rowmid) {
				...
				result = binarySearchMatrix(matrix, target, rows, rowmid-1, cols, colmid-1) || binarySearchMatrix(matrix, target, rows, rowmid-1, colmid+1, cole) || binarySearchMatrix(matrix, target, rowmid+1, rowe, cols, colmid-1)
			}
		} else {
			//fmt.Println("else")
			if binarySearchRows(matrix, target, rowmid, colmid, cole) {
				...
				result = binarySearchMatrix(matrix, target, rowmid+1, rowe, cols, colmid-1) || binarySearchMatrix(matrix, target, rowmid+1, rowe, colmid+1, cole) || binarySearchMatrix(matrix, target, rows, rowmid-1, colmid+1, cole)
			}
		}
	}
	return result
